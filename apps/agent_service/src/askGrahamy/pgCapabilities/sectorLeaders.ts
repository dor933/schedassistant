...
export type SectorLeadersOptions = {
  queryRunner?: (
...
export async function buildSectorLeadersView(
  ...
  if (!options.queryRunner && !isExternalPgConfigured()) {
    return {
      views: {
        stockIdeaView: unavailableView(rankingBasis, [
          "External PG warehouse is not configured for sector-leaders queries.",
        ]),
      },
      warnings: [],
    };
  ...
  try {
    ...
    const draft = rowsToView(rows, rankingBasis, sector, options.now);
    ...
    const message = "Sector leaders query failed.";
    logger.warn("Ask Grahamy PG capability failed", {
      capability: "sector_leaders",
...
function rowsToView(
  ...
  if (!rows.length) {
    return unavailableView(rankingBasis, [
      `No leading-stock rows were available for ${sector} from the PG warehouse.`,
    ]);
  }
  ...
  if (!publicRows.length) {
    return unavailableView(rankingBasis, [
      `No public-safe sector-leaders rows could be derived for ${sector}.`,
    ]);
  }
  ...
  if (freshnessAssessment.decision === "unavailable") {
    return unavailableView(
      rankingBasis,
      [freshness.warning ?? `Sector-leaders data for ${sector} is stale.`],
      freshness,
    );
  }
  const warnings: string[] = [
    `Sector-internal ranking for ${sector}; rows are research candidates to review, not buy/sell recommendations.`,
  ...
  if (freshness.state === "stale") {
    warnings.push(
      freshness.warning ??
        "This sector-leaders view uses stale data and should be treated as a snapshot.",
    );
...
function targetSector(input: PgCapabilityRunInput): string | undefined {
  const fromClassification = input.classification.sectors[0];
