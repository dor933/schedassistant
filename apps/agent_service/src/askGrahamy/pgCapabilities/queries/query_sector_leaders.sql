-- Sector-internal "leading stocks" ranking. Mirrors query_stock_idea_discovery.sql
-- but pre-filters to a single sector (case-insensitive) and ranks all candidates
-- within that sector instead of diversifying across sectors. Returns empty when
-- :SECTOR_FILTER is null/empty so the capability builder can fall back to an
-- unavailable view.
WITH config AS (
  SELECT
    ...
    NULLIF(TRIM(CAST(:SECTOR_FILTER AS text)), '') AS sector_filter
),
current_rows AS (
  SELECT
    f.symbol,
  ...
  WHERE ms.is_active = true
    ...
    AND c.sector_filter IS NOT NULL
    AND LOWER(p.sector) = LOWER(c.sector_filter)
),
scored AS (
  SELECT
    c.*,
...
ranked AS (
  -- Single-sector ranking — all rows belong to one sector, so partitioning by
  -- sector reduces to a global ORDER BY across the filtered pool. We keep the
  -- structure parallel to query_stock_idea_discovery for readability.
  SELECT
    cp.*,
    fo.sample_size,
    fo.hit_rate_pct,
