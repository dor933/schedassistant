...
SELECT
  JSONB_BUILD_OBJECT(
    ...
    'path_risk_base', JSONB_BUILD_OBJECT(
      ...
      'horizon',                       '60-day',
      'n',                             (SELECT path_n FROM regime_path_risk),
      'loss_rate_h60_pct',             (SELECT ROUND(loss_rate_h60_pct::numeric, 2) FROM regime_path_risk),
      'severe_loss_rate_h60_pct',      (SELECT ROUND(severe_loss_rate_h60_pct::numeric, 2) FROM regime_path_risk),
      'p10_max_drawdown_pct',          (SELECT ROUND(p10_max_drawdown_pct::numeric, 2) FROM regime_path_risk),
      'p25_max_drawdown_pct',          (SELECT ROUND(p25_max_drawdown_pct::numeric, 2) FROM regime_path_risk),
      'worst_max_drawdown_pct',        (SELECT ROUND(worst_max_drawdown_pct::numeric, 2) FROM regime_path_risk),
      'prob_drawdown_gt_5_pct',        (SELECT ROUND(prob_drawdown_gt_5_pct::numeric, 2) FROM regime_path_risk),
      'prob_drawdown_gt_10_pct',       (SELECT ROUND(prob_drawdown_gt_10_pct::numeric, 2) FROM regime_path_risk),
      'prob_drawdown_gt_15_pct',       (SELECT ROUND(prob_drawdown_gt_15_pct::numeric, 2) FROM regime_path_risk),
      'prob_drawdown_gt_20_pct',       (SELECT ROUND(prob_drawdown_gt_20_pct::numeric, 2) FROM regime_path_risk),
      'median_recovery_days',          (SELECT ROUND(median_recovery_days::numeric, 1) FROM regime_path_risk),
      'recovered_by_horizon_rate_pct', (SELECT ROUND(recovered_by_horizon_rate_pct::numeric, 2) FROM regime_path_risk),
      'sample_adequacy',               (SELECT sample_adequacy FROM regime_path_risk)
    ),
