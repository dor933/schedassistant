...
diversified AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ORDER BY
        CASE WHEN (SELECT rank_by FROM config) = 'historical_forward'
          THEN r.hit_rate_pct END DESC NULLS LAST,
        CASE WHEN (SELECT rank_by FROM config) = 'risk_adjusted'
          THEN COALESCE(r.median_return_pct, 0) - ABS(COALESCE(r.p25_return_pct, 0)) END DESC NULLS LAST,
        CASE WHEN (SELECT rank_by FROM config) = 'conviction'
          THEN r.composite_pct * 100 END DESC NULLS LAST,
        r.setup_score DESC NULLS LAST,
        r.avg_dollar_volume DESC NULLS LAST,
        r.symbol ASC
    ) AS stock_rank
  FROM ranked r
  WHERE r.sector_rank <= 2
)
SELECT
  d.symbol,
  d.company_name,
