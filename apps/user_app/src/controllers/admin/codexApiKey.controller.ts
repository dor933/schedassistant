...
import { CodexApiKeyService } from "../../services/admin/codexApiKey.service";
import { logger } from "../../logger";

function handleError(res: Response, err: any, scope: string) {
  if (err?.status) return res.status(err.status).json({ error: err.message });
  logger.error(`Admin codex-api-key ${scope} error`, { error: err?.message });
  return res.status(500).json({ error: "Internal server error." });
}

export class CodexApiKeyController {
  private service = new CodexApiKeyService();
