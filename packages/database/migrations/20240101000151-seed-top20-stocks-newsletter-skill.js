...
const SKILL_TEXT = [
  ...
  "- Call `get_latest_newsletter_issue` with `type: \"top20_stocks\"` before building this issue. If it returns a `summary`, use that as previous-newsletter context when preparing the new `changeSummary` field. If no issue exists, continue without previous context.",
  "",
  ...
  "",
  "8. Record the newsletter issue pointer.",
  "",
  "After the send succeeds, record the issue from the primary agent using the newsletter issue tools:",
  "",
  "- If this is the first Top 20 newsletter record for the issue date, call `record_newsletter_issue` with `type: \"top20_stocks\"`, `issueDate` as `YYYY-MM-DD`, `fileUri` pointing to the saved cleaned payload file, `payloadUri` pointing to the same file if no separate payload file exists, `subject`, `asOfDate`, `summary` as the concise change summary / previous-context note, and Gmail `sentMessageId` / `sentThreadId` from the send result.",
  ...
  "- If you do not have an issue id but know this is a same-day replacement, call `update_newsletter_issue_path` with `type: \"top20_stocks\"`, the same `issueDate`, and the new `fileUri` / `payloadUri`; it updates the latest same-day record for this organization and type.",
  "",
  "## Quality bar",
  "",
