 ...
 * skill-driven guidance that describes the **two possible file-tool surfaces**
 * an agent might be running on:
 *
 *  - **A — Native SDK file tools** (`Read` / `Write` / `Edit` / `MultiEdit` /
 *    `Glob` / `Grep`, capitalised). Used by Anthropic-SDK paths with built-ins
 *    enabled (primary agents with `allow_sdk_builtins=true`, deep-agent
 *    executors on the SDK path). Resolution is relative to the spawned
 *    subprocess's cwd; a `PreToolUse` hook enforces the `.md`/`.txt`
 *    extension gate before disk.
 *  - **B — Filesystem MCP tools** (`read_text_file` / `write_file` /
 *    `edit_file` / `list_directory` / `search_files`, lowercase). Used by
 *    Codex SDK paths (filesystem MCP exposed via the bridge) and any path
 *    where the agent has filesystem MCP attached but SDK built-ins
 *    disabled. Absolute paths under `/app/data`; `instrumentFsWriteTools`
 *    enforces the same extension gate.
 *
 * The skill text describes both surfaces and tells the model to pick by
 * inspecting its own tool list. Same content guarantees in either path:
 * `.md`/`.txt` only, sandboxed to the agent's workspace, manifest capture
 * for per-thread folder writes.
 *
 * Two skills are written:
 *
 *  - `dev-in-house-workspace` — the agent's persistent workspace,
 *    described dual-mode.
 *
 *  - `dev-in-house-library-mcp` — the org library; available on the MCP
 *    path always, on the SDK path only when a library MCP is attached.
 *
